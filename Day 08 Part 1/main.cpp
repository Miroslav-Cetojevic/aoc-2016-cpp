...
int main() {
	...
	if(file.is_open()) {
		...
		auto num_px_lit = std::accumulate(screen.begin(), screen.end(), 0UL, [] (auto acc, const auto& row) {
			return acc + std::accumulate(row.begin(), row.end(), 0UL, [] (auto acc, const auto col) {
				return acc + col;
			});
		});

		std::cout << num_px_lit << std::endl;
